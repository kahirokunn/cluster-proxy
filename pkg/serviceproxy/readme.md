...
The top-level `enableHubTokenAuthentication` Helm value grants the required
hub permissions. To control hub TokenReview authentication per managed cluster,
set the `enableHubTokenAuthentication` AddOnDeploymentConfig variable, which
maps to `--enable-hub-token-authentication`. When unset, it inherits the
deprecated `enableImpersonation` value; when both are set,
...
Use `--oidc-ca-file` for a static CA bundle or `--oidc-ca-configmap` for a
dynamically reconciled ConfigMap; the two flags are mutually exclusive. Run
`cluster-proxy service-proxy --help` to inspect the CLI defaults.
...
`ca.crt` disables only OIDC authentication; creating or correcting the
ConfigMap initializes it without a Pod restart. Deleting the ConfigMap fails
OIDC authentication closed and cancels the previous authenticator.
