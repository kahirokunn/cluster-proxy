...
func TestTokenReviewAuthenticator_StatusError_ReturnsUnauthenticated(t *testing.T) {
	// A completed TokenReview that did not authenticate the token is the cluster's
	// definitive rejection, regardless of how Status.Error is worded. The
	// authenticator reports it as unauthenticated (nil, false, nil) so the caller
	// can fall back to another cluster.
	tests := []struct {
		name        string
		statusError string
	}{
