...
func observeStepFailure(err error) (deleting bool) {
	if errors.Is(err, errAddonDeleting) {
		addonmetrics.ObserveManagedKubeconfigRefresh("skipped")
		return true
	}
...
func BuildManagedKubeconfig(sourceKubeconfig []byte, token string) ([]byte, error) {
	...
	clusterCopy := *cluster
	if err := flattenClusterCA(&clusterCopy); err != nil {
		return nil, err
	}
...
// flattenClusterCA inlines a file-based certificate-authority reference into
// CertificateAuthorityData so the generated kubeconfig remains self-contained
// when consumed from a Secret on a pod that does not share the source pod's
// filesystem.
func flattenClusterCA(cluster *clientcmdapi.Cluster) error {
	if cluster.CertificateAuthority == "" {
		return nil
	}
	data, err := os.ReadFile(cluster.CertificateAuthority)
	if err != nil {
		return fmt.Errorf("read certificate-authority file %q: %w", cluster.CertificateAuthority, err)
	}
