...
const (
	// ComponentName identifies the provisioner as a client user-agent, event
	// source, and reporting controller.
	ComponentName = "cluster-proxy-managed-kubeconfig-provisioner"
...
var errAddonDeleting = errors.New("managedclusteraddon is deleting")
...
func (o *Options) Complete() {
	...
	if o.AddonName == "" {
		o.AddonName = DefaultAddonName
	}
	if o.AddonNamespace == "" {
		o.AddonNamespace = o.ClusterName
	}
	if o.HealthProbeBindAddress == "" {
		o.HealthProbeBindAddress = fmt.Sprintf(":%d", constant.ManagedKubeconfigProvisionerHealthProbePort)
	}
}

// Validate rejects Options that Complete cannot make usable: a missing
// ClusterName or TargetNamespace and inconsistent duration settings.
func (o Options) Validate() error {
	...
	// RefreshBefore must be strictly less than TokenExpiration, otherwise every
	// freshly-issued token is immediately treated as stale.
	if o.RefreshBefore >= o.TokenExpiration {
		return fmt.Errorf("refresh before (%s) must be less than token expiration (%s)", o.RefreshBefore, o.TokenExpiration)
	}
	if o.SyncInterval <= 0 {
...
func (p *Provisioner) cleanupIfAddonDeleting(ctx context.Context) error {
	deleting, err := p.addonDeleting(ctx)
	if err != nil {
		return err
	}
	if !deleting {
	...
	return errAddonDeleting
}

// addonClientConfigured reports whether the provisioner has an addon client and
// the ManagedClusterAddOn coordinates needed to read or patch its status.
func (p *Provisioner) addonClientConfigured() bool {
	return p.addonClient != nil && p.options.AddonNamespace != "" && p.options.AddonName != ""
}

func (p *Provisioner) addonDeleting(ctx context.Context) (bool, error) {
	if !p.addonClientConfigured() {
		return false, nil
	}
	addon, err := p.addonClient.AddonV1alpha1().ManagedClusterAddOns(p.options.AddonNamespace).Get(
		ctx, p.options.AddonName, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
		return true, nil
	}
	if err != nil {
	...
	return !addon.DeletionTimestamp.IsZero(), nil
}

// observeStepFailure records the refresh-result metric for a failed Sync step
// and reports whether err is the benign addon-deleting sentinel, which Sync
// treats as success.
func observeStepFailure(err error) (deleting bool) {
	if errors.Is(err, errAddonDeleting) {
		addonmetrics.ObserveManagedKubeconfigRefresh(addonmetrics.ResultSkipped)
		return true
	}
	addonmetrics.ObserveManagedKubeconfigRefresh(addonmetrics.ResultError)
	return false
}

func (p *Provisioner) Sync(ctx context.Context) error {
	if err := p.cleanupIfAddonDeleting(ctx); err != nil {
		if observeStepFailure(err) {
			return nil
		}
		p.recordEvent(ctx, corev1.EventTypeWarning, "ManagedKubeconfigCleanupFailed", err.Error())
		return err
	...
	if err != nil {
		...
		if conditionErr := p.setManagedKubeconfigCondition(ctx, metav1.ConditionFalse, "SyncFailed", err.Error()); conditionErr != nil {
...
func (p *Provisioner) sync(ctx context.Context) (syncResult, error) {
	...
	if targetExists && !needsRefresh(target, sourceHash, p.now(), p.options.RefreshBefore) {
		...
		return syncResult{
			metricResult: addonmetrics.ResultSkipped,
	...
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				annotationTokenExpirationTimestamp: expiresAt,
	...
	klog.Infof("managed kubeconfig secret %s/%s synced; token expires at %s",
		p.options.TargetNamespace, p.options.TargetName, expiresAt)
	reason := "ManagedKubeconfigCreated"
	if targetExists {
		reason = "ManagedKubeconfigUpdated"
	}
	return syncResult{
		metricResult: addonmetrics.ResultSuccess,
		reason:       reason,
...
func (p *Provisioner) reportingInstance() string {
	...
	// recordEvent, the only caller, guarantees both fields are set.
...
func (p *Provisioner) setManagedKubeconfigCondition(ctx context.Context, status metav1.ConditionStatus, reason, message string) error {
	...
	// The addon-manager writes its own conditions to the same ManagedClusterAddOn
	// status subresource, so a plain Get/UpdateStatus race-loses to its writes
	// with a conflict. Re-read the latest object on each attempt and retry so
	// our condition lands without overwriting concurrent updates.
...
func BuildManagedKubeconfig(sourceKubeconfig []byte, token string) ([]byte, error) {
	...
	cluster, err := utils.CurrentCluster(sourceConfig)
	if err != nil {
		return nil, err
	}

	clusterCopy := *cluster
	// Inline any file-based certificate-authority reference into
	// CertificateAuthorityData so the generated kubeconfig stays self-contained
	// when consumed from a Secret on a pod that does not share the source pod's
	// filesystem.
	if err := clientcmdapi.FlattenContent(&clusterCopy.CertificateAuthority, &clusterCopy.CertificateAuthorityData, ""); err != nil {
		return nil, fmt.Errorf("flatten managed cluster certificate-authority: %v", err)
	}
...
func needsRefresh(secret *corev1.Secret, sourceHash string, now time.Time, refreshBefore time.Duration) bool {
	...
	expiration := tokenExpirationFromSecret(secret)
	if expiration.IsZero() {
		return true
	}
...
func newManagedClient(kubeconfig []byte) (kubernetes.Interface, error) {
	...
	return kubernetes.NewForConfig(rest.AddUserAgent(config, ComponentName))
}
