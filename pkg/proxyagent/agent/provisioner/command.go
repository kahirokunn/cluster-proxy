...
func NewManagedKubeconfigProvisionerCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			hostingClient, err := kubernetes.NewForConfig(rest.AddUserAgent(config, ComponentName))
			if err != nil {
				return err
			}
			...
			if options.HubKubeconfig != "" {
				...
				hubAddonClient, err := addonclient.NewForConfig(rest.AddUserAgent(hubConfig, ComponentName))
				if err != nil {
					return err
				}
	...
	flags.StringVar(&options.SourceName, "source-name", options.SourceName, "The external managed kubeconfig secret name")
	...
	flags.StringVar(&options.TargetName, "target-name", options.TargetName, "The generated managed kubeconfig secret name")
	...
	flags.StringVar(&options.ManagedServiceAccountName, "managed-service-account-name", options.ManagedServiceAccountName, "The managed cluster service account name")
	flags.DurationVar(&options.TokenExpiration, "token-expiration", options.TokenExpiration, "Requested TokenRequest expiration")
	flags.DurationVar(&options.RefreshBefore, "refresh-before", options.RefreshBefore, "Refresh the managed kubeconfig this long before token expiration")
	flags.DurationVar(&options.SyncInterval, "sync-interval", options.SyncInterval, "Interval between managed kubeconfig syncs")
	...
	flags.StringVar(&options.AddonName, "addon-name", options.AddonName, "The ManagedClusterAddOn name to patch with managed kubeconfig readiness")
	...
	flags.StringVar(&options.HealthProbeBindAddress, "health-probe-bind-address", options.HealthProbeBindAddress, "The address the health probe and metrics endpoint binds to")
