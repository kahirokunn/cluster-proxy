...
// to pick up a new apiserver address.
...
func NewCommand() *cobra.Command {
	proxy := &Proxy{
		Listen:                 fmt.Sprintf(":%d", constant.ManagedAPIServerProxyPort),
		DialTimeout:            30 * time.Second,
		HealthProbeBindAddress: fmt.Sprintf(":%d", constant.ManagedAPIServerProxyHealthProbePort),
...
func (p *Proxy) loadTarget() (string, error) {
	config, err := clientcmd.LoadFromFile(p.ManagedKubeconfig)
	if err != nil {
		return "", err
	}
	cluster, err := utils.CurrentCluster(config)
	if err != nil {
		return "", err
	}
	return targetAddress(cluster.Server)
}
