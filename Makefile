...
.PHONY: test-helm
test-helm: verify-helm-dependencies validate-values-schema verify-ha-manifests ## Lint and render Helm charts.
...
verify-ha-manifests:
	...
	test "$$(grep -c 'topologySpreadConstraints:' "$$tmp/hub-single.yaml")" -eq 1; \
	$(HELM) template cluster-proxy charts/cluster-proxy \
		--namespace open-cluster-management-addon \
		--set replicas=2 \
	...
	test "$$(grep -c 'topologySpreadConstraints:' "$$tmp/agent-single.yaml")" -eq 1; \
	$(HELM) template addon-agent pkg/proxyagent/agent/manifests/charts/addon-agent \
		--namespace open-cluster-management-agent-addon \
