...
var _ = Describe("Requests through Cluster-Proxy", Label("serviceproxy", "connectivity"), func() {
	...
	Describe("Watch ConfigMap create", Label("watch"), func() {
		It("shoud watch", Label("configmap"), func() {
			...
			Eventually(watcher.ResultChan(), timeout, time.Second).Should(Receive(Satisfy(func(event k8swatch.Event) bool {
				configMap, ok := event.Object.(*corev1.ConfigMap)
				return event.Type == k8swatch.Added && ok && configMap.Name == "cluster-proxy-test"
			})))
