...
require_port_available() {
	...
	if port_is_open "${port}"; then
		echo "localhost:${port} is already in use; choose a different port with PROXY_ENTRYPOINT_LOCAL_PORT or USER_SERVER_LOCAL_PORT" >&2
		exit 1
	fi
}
...
wait_for_port_forward() {
	...
	for _ in $(seq 1 120); do
		if ! kill -0 "${pid}" >/dev/null 2>&1; then
			echo "port-forward process for localhost:${port} exited early" >&2
			cat "${log_file}" >&2 || true
			exit 1
		fi
		if port_is_open "${port}" && grep -q "Forwarding from 127.0.0.1:${port}" "${log_file}"; then
			return
		fi
		sleep 1
	done
...
start_port_forward() {
	...
	require_port_available "${local_port}"
	...
	local pid="$!"
	PIDS+=("${pid}")
	wait_for_port_forward "${local_port}" "${pid}" "${log_file}"
}
