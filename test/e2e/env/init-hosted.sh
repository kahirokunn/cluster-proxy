...
wait_addon_finalizer() {
	local finalizer="$1"
	local timeout="${2:-300}"

	log "Waiting for hosted ManagedClusterAddOn finalizer ${finalizer}"
	for _ in $(seq 1 "${timeout}"); do
		if kubectl --kubeconfig "${HUB_KUBECONFIG}" -n "${MANAGED_CLUSTER_NAME}" get managedclusteraddon cluster-proxy \
			-o go-template='{{range .metadata.finalizers}}{{.}}{{"\n"}}{{end}}' | grep -qx "${finalizer}"; then
			return
		fi
		sleep 1
	done

	kubectl --kubeconfig "${HUB_KUBECONFIG}" -n "${MANAGED_CLUSTER_NAME}" get managedclusteraddon cluster-proxy -o yaml || true
	echo "Timed out waiting for hosted ManagedClusterAddOn finalizer ${finalizer}" >&2
	exit 1
}
...
install_cluster_proxy() {
	...
	kubectl --kubeconfig "${HUB_KUBECONFIG}" -n "${HUB_NAMESPACE}" patch service proxy-entrypoint --type=merge -p "{
		\"spec\": {
			\"type\": \"NodePort\",
			\"ports\": [
				{\"name\":\"proxy-server\",\"port\":8090,\"nodePort\":${PROXY_SERVER_NODE_PORT}},
				{\"name\":\"agent-server\",\"port\":8091,\"nodePort\":${PROXY_ENTRYPOINT_NODE_PORT}}
			]
		}
	}" >/dev/null
}
...
wait_hosted_addon_ready() {
	...
	for _ in $(seq 1 120); do
		if kubectl --kubeconfig "${HUB_KUBECONFIG}" -n "${MANAGED_CLUSTER_NAME}" get managedclusteraddon cluster-proxy \
			...
			wait_addon_finalizer "${HOSTING_MANIFEST_FINALIZER}"
			wait_addon_finalizer "${HOSTING_PRE_DELETE_HOOK_FINALIZER}"
			return
		fi
		sleep 5
	done
...
main() {
	...
	local image="${IMAGE_REGISTRY_NAME}/${IMAGE_NAME}:${IMAGE_TAG}"
	# `kind load docker-image` only reads from the Docker daemon's image store,
	# so for non-docker engines (e.g. podman, nerdctl) we export the image to a
	# tarball and load it via `kind load image-archive` instead.
	if [[ "${CONTAINER_ENGINE}" == "docker" ]]; then
		...
		local archive="${WORK_DIR}/cluster-proxy-image.tar"
		log "Exporting ${image} from ${CONTAINER_ENGINE} to ${archive}"
		"${CONTAINER_ENGINE}" save -o "${archive}" "${image}"
		for cluster in "${HUB_CLUSTER_NAME}" "${HOSTING_CLUSTER_NAME}" "${MANAGED_CLUSTER_NAME}"; do
			log "Loading ${archive} into ${cluster}"
